...
type
  ...
  TRasPhonebook = class;
  TRasConnectionItem = class;

  TRasConnection = class
  private const
    MAX_BUSYTIMEOUT: Cardinal = 30000;
  private
    FBusy: Boolean;
    FRasConn: TRasConn;
    FChanged: Boolean;
    FBusyEndTime: Cardinal;
  private
  ...
  private
    ...
    procedure SetRasConn(const Value: TRasConn);
  ...
  public
    ...
    property RasConn: TRasConn read FRasConn write SetRasConn;
  ...
  public
    ...
    function SetProperties(const Value: PRasEntry; Size: DWORD;
      FreeValue: Boolean = False): DWORD;
    property CallbackNumber: String read GetCallbackNumber
      write SetCallbackNumber;
    ...
    property NeedConnect: Boolean read FNeedConnect write FNeedConnect;
    property Connection: TRasConnection read FConnection write FConnection;
  end;
  ...
  TRasDialerEvent = procedure(Sender: TObject; State: TRasConnState;
...
constructor TRasPhonebookEntry.Create(AOwner: TRasPhonebook;
  const AName: String);
begin
  inherited Create;
  FName := AName;
  ...
  FConnection := TRasConnection.Create;
  FNeedConnect := False;
end;
...
destructor TRasPhonebookEntry.Destroy;
begin
...
begin
  Result := 0;
  Res := RasGetEntryProperties(FOwner.FPBK, PWideChar(FName), nil, Result,
    nil, nil);
  if Res <> ERROR_BUFFER_TOO_SMALL then
    RasCheck(Res);
  Value := AllocMem(Result);
  Value^.dwSize := Sizeof(TRasEntry);
  Res := RasGetEntryProperties(FOwner.FPBK, PWideChar(FName), Value, Result,
    nil, nil);
  if Res <> SUCCESS then
  begin
    FreeMem(Value);
...
begin
  RasCheck(RasCreatePhonebookEntry(Wnd, FPBK));
  Refresh;
end;
...
constructor TRasConnectionItem.Create(AOwner: TRasConnectionsList;
  ARasConn: TRasConn);
begin
  inherited Create;
  FOwner := AOwner;
...
function TRasConnectionItem.GetIPAddress: string;
var
  dwSize: Cardinal;
  RASpppIP: TPRasPppIP;
begin
  ...
  If RASGetProjectionInfo(FRasConn.hrasconn, RASP_PPPIP, @RASpppIP,
    dwSize) = 0 Then
...
begin
  ...
  if FNotifyMessage = 0 then
    FNotifyMessage := WM_RASDIALEVENT;
...
begin
  FConnHandle := 0;
  RasCheck(RasDial(nil, nil, @FParams, $FFFFFFFF, Pointer(FNotifyWnd),
    FConnHandle));
end;
...
begin
  with Message do
  begin
    if WParam = RASCS_Disconnected then
      FConnHandle := 0;
    if Assigned(FOnNotify) then
      FOnNotify(Self, WParam, LParam);
  end;
end;
...
function TRasConnection.GetBusy: Boolean;
begin
  if FBusy and (GetTickCount > FBusyEndTime) then
    FBusy := False;
  Result := FBusy;
end;

function TRasConnection.GetConnStatus: TRasConnStatus;
begin
  ZeroMemory(@Result, Sizeof(Result));
  Result.dwSize := Sizeof(Result);
  RasCheck(RasGetConnectStatus(FRasConn.hrasconn, Result));
...
function TRasConnection.GetConnStatusStr: String;
begin
  with GetConnStatus do
    Result := RasConnStatusString(rasconnstate, dwError);
end;

function TRasConnection.GetDeviceName: String;
begin
  Result := FRasConn.szDeviceName;
end;

function TRasConnection.GetDeviceType: String;
begin
  Result := FRasConn.szDeviceType;
end;

function TRasConnection.GetHandle: THandle;
begin
  Result := FRasConn.hrasconn;
end;

function TRasConnection.GetIPAddress: string;
var
  dwSize: Cardinal;
  RASpppIP: TPRasPppIP;
begin
...
function TRasConnection.GetIsConnected: Boolean;
begin
  Result := Handle <> 0;
end;

function TRasConnection.GetName: String;
begin
  Result := FRasConn.szEntryName;
end;

procedure TRasConnection.HangUp(WaitToCompleteSec: Integer);
begin
  if Handle <> 0 then
  begin
    ConnectionHangUp(Handle, WaitToCompleteSec);
    Handle := 0;
  end;
end;

procedure TRasConnection.SetBusy(const Value: Boolean);
begin
  FBusy := Value;
  FBusyEndTime := GetTickCount + MAX_BUSYTIMEOUT;
end;

procedure TRasConnection.SetHandle(const Value: THandle);
begin
  if Value <> FRasConn.hrasconn then
    FChanged := True;
  FRasConn.hrasconn := Value;
end;

procedure TRasConnection.SetRasConn(const Value: TRasConn);
begin
